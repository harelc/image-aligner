#!/usr/bin/env python3
"""
Image Aligner - Gradio Web Interface
...
def process_images(source_img, target_img):
    ...
    # Align
    aligned, naive_resized, _ = align_image(source_bgr, target_bgr)

    # Create visualization panel
    panel = create_visualization_panel(naive_resized, aligned, target_bgr, "Alignment Result")

    # Convert back to RGB for Gradio
    aligned_rgb = cv2.cvtColor(aligned, cv2.COLOR_BGR2RGB)
    panel_rgb = cv2.cvtColor(panel, cv2.COLOR_BGR2RGB)

    return panel_rgb, aligned_rgb
...
<div class="dedication">
    <h2>Dedicated with <span class="heart">♥</span> love and devotion to</h2>
...
# Build the interface
with gr.Blocks(css=custom_css, title="Image Aligner", theme=gr.themes.Soft()) as demo:
    gr.HTML(dedication_html)

    gr.Markdown("# Image Aligner\nGeometric alignment with background-aware color matching")

    with gr.Row():
        with gr.Column():
            source_input = gr.Image(label="Source Image (to align)", type="numpy")
        with gr.Column():
            target_input = gr.Image(label="Target Reference", type="numpy")

    align_btn = gr.Button("✨ Align Images", variant="primary", size="lg")

    gr.Markdown("## Results")

    panel_output = gr.Image(label="Visualization Panel", type="numpy")
    aligned_output = gr.Image(label="Aligned Result (download this)", type="numpy")

    align_btn.click(
        process_images,
        inputs=[source_input, target_input],
        outputs=[panel_output, aligned_output],
        api_name="align"
    )
